...
def _reap(proc: subprocess.Popen[bytes], pf: pidfd.PidFd | None) -> None:
...
def test_open_self() -> None:
    with pidfd.PidFd.open(os.getpid()) as pf:
        assert pf.fileno() > 0
...
def test_open_nonexistent_pid() -> None:
    with pytest.raises(pidfd.PidFdError) as excinfo:
        pidfd.PidFd.open(2**30)
...
def test_open_bad_flags() -> None:
    with pytest.raises(pidfd.PidFdError) as excinfo:
        pidfd.PidFd.open(os.getpid(), 1 << 20)
    assert excinfo.value.errno == errno.EINVAL


@requires_linux
def test_open_nonblock_flag() -> None:
    with pidfd.PidFd.open(os.getpid(), pidfd.OpenFlag.NONBLOCK) as pf:
...
def test_wrap_existing_fd() -> None:
    fd = pidfd._syscall.pidfd_open(os.getpid(), 0)
    with pidfd.PidFd(fd) as pf:
        assert pf.fileno() == fd
...
def test_poll_readable_after_exit() -> None:
    proc = _spawn()
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            proc.kill()
            poller = select.poll()
...
def test_signal_kills_child() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            ...
            assert result.code == pidfd.WaitCode.CLD_KILLED
...
def test_wait_reports_exit_status() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            ...
            assert result.signo == signal.SIGCHLD
            assert result.pid == proc.pid
            assert result.code == pidfd.WaitCode.CLD_EXITED
...
def test_wait_wnohang_on_running_child() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            result = pf.wait(pidfd.WaitFlag.WEXITED | pidfd.WaitFlag.WNOHANG)
...
def test_wait_nonblock_pidfd_raises_eagain() -> None:
    proc = _spawn()
    try:
        with pidfd.PidFd.open(proc.pid, pidfd.OpenFlag.NONBLOCK) as pf:
            with pytest.raises(pidfd.PidFdError) as excinfo:
                pf.wait()
...
def test_wait_non_child_raises_echild() -> None:
    with pidfd.PidFd.open(os.getpid()) as pf:
        with pytest.raises(pidfd.PidFdError) as excinfo:
            pf.wait(pidfd.WaitFlag.WEXITED | pidfd.WaitFlag.WNOHANG)
...
def test_pidfd_send_signal_convenience() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            pidfd.pidfd_send_signal(pf, signal.SIGKILL)
            result = pf.wait()
            assert result.code == pidfd.WaitCode.CLD_KILLED
    finally:
        _reap(proc, None)
...
def test_pidfd_send_signal_raw_fd() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            pidfd.pidfd_send_signal(pf.fileno(), signal.SIGKILL)
            result = pf.wait()
            assert result.code == pidfd.WaitCode.CLD_KILLED
    finally:
        _reap(proc, None)
...
def test_pidfd_send_signal_proc_fd() -> None:
    ...
    try:
        ...
        try:
            pidfd.pidfd_send_signal(fd, signal.SIGKILL)
        finally:
            os.close(fd)
...
def test_getfd() -> None:
    ...
    try:
        with pidfd.PidFd.open(proc.pid) as pf:
            try:
                dup = pf.getfd(0)
            except pidfd.PidFdError as exc:
